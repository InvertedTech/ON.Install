...
try
{
    ResponseTransaction result = await transactionsReadController.GetTransactionAsync(transactionId);
}
catch (ApiException e)
{
    // TODO: Handle exception here
...
ListTransactionsAsync(
    ...
    List<Models.Field52Enum> fields = null)
```
...
| `fields` | [`List<Field52Enum>`](../../doc/models/field-52-enum.md) | Query, Optional | You can use any `field_name` from this endpoint results to filter the list of fields returned on the response. |
...
try
{
    ResponseTransactionsCollection result = await transactionsReadController.ListTransactionsAsync(
        page,
        order,
        filterBy
    );
